...
/*Because branch and jump instructions can flush the pipeline, so the exact cycle when the result appears may vary.
Instead of checking at a fixed cycle, it checks whenever the expected result appears.*/
...
class Assignment5BranchTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  private val wrongPathMarkers = (901 to 918).map(BigInt(_)).toSet   /*901-918 are wrong-path markers used to verify 
                                          that the processor flushes incorrectly fetched instructions after a taken branch or jump| means branch/jump failed ,pipeline wasn't flushed correctly ,wrong-path instruction executed.*/

//This function waits until the processor outputs the expected result.
